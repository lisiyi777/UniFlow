...
# If you find this repo helpful, please cite the respective publication
# listed on the above website.
#
# Modified by Siyi Li – 2025-12-28
# Changes: inserted beam ID information into the preprocessed outputs
# to support downstream scan-pattern augmentation.
#
# Description: Preprocess data and save as HDF5 format for faster loading
# Reference:
#   * ZeroFlow data preprocessing: https://github.com/kylevedder/argoverse2-sf
#   * Argoverse API: https://github.com/argoverse/av2-api
...
def insert_beam_id_av2(
    preprocess_dir: str,
    raw_sensor_root: str,
    overwrite: bool = True,
):
    """
    Insert per-point beam ids (from AV2 'laser_number') into existing .h5 files.
    """

    pre_dir = Path(preprocess_dir)
    raw_root = Path(raw_sensor_root)

    if not pre_dir.exists():
        print(f"[insert_beam_id_av2] preprocess_dir not found: {pre_dir}")
        return
    if not raw_root.exists():
        print(f"[insert_beam_id_av2] raw_sensor_root not found: {raw_root}")
        return

    h5_files = sorted([p for p in pre_dir.iterdir() if p.suffix == ".h5"])

    print(f"[insert_beam_id_av2] Found {len(h5_files)} scene files in {pre_dir}")
    changed_files = 0
    skipped_files = 0
    total_groups = 0
    updated_groups = 0
    ...
    for h5_path in tqdm(h5_files, desc="Inserting beam_id", ncols=100):
        log_id = h5_path.stem
        raw_log_dir = raw_root / log_id / "sensors" / "lidar"
        ...
        with h5py.File(h5_path, "a") as f:
            ...
            for ts in ts_keys:
                ...
                feather_path = raw_log_dir / f"{ts}.feather"
                if not feather_path.exists():
                    skipped_groups += 1
                    continue

                try:
                    df = pd.read_feather(feather_path, columns=["laser_number"])
                except Exception as e:
                    skipped_groups += 1
                    continue

                beam_id = df["laser_number"].to_numpy(dtype=np.uint8)
    ...
    print("\n[insert_beam_id_av2] Summary")
    print(f"  Scenes processed:   {len(h5_files)}")
...
def main(
    ...
    if only_insert_beamid:
        insert_beam_id_av2(output_dir_, data_root_)
