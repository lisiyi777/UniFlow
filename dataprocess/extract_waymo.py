...
# If you find this repo helpful, please cite the respective publication listed on the above website.
#
# Modified by Siyi Li – 2025-12-28
# Changes:
#     Added per-point beam_id extraction for Waymo TOP lidar (range image row index)
#     and stored it in the preprocessed outputs as dataset "beam_id".
#
# Description: Preprocess Waymo Open Dataset scene flow TFRecords and save as HDF5 for faster loading.
# This script is heavily based on Kyle Vedder's ZeroFlow Waymo preprocessing:
#   https://github.com/kylevedder/zeroflow/tree/master/data_prep_scripts/waymo
#
# Environment:
...
def convert_range_image_to_point_cloud(
    ...
    for c in calibrations:
        ...
        rc = tf.compat.v1.where(range_image_mask)

        range_image_cartesian = cartesian_range_images[c.name]
        points_tensor = tf.gather_nd(range_image_cartesian, rc)
        ...
        beam_ids.append(row_idx.astype(np.uint8, copy=False))

    return points, cp_points, flows, beam_ids
...
def get_car_pc_global_pc_flow_transform(frame: dataset_pb2.Frame):
    ...
    points_lst, cp_points, flows_lst, beam_ids_lst = convert_range_image_to_point_cloud(
        frame,
        range_images,
        camera_projections,
        point_flows,
...
def process_log(data_dir: Path, log, log_map_folder, output_dir: Path, n = None) :
    ...
    def create_group_data(group, pc, pose, gm = None, flow_0to1=None, flow_valid=None, flow_category=None, ego_motion=None, beam_id=None):
        ...
        if beam_id is not None:
            group.create_dataset('beam_id', data=beam_id.astype(np.uint8))
    ...
    for data_idx in range(1, total_lens):
        ...
        if scene_id != frame.context.name:
            ...
            break
        car_frame_pc, global_frame_pc, flow, label, pose, beam_id = get_car_pc_global_pc_flow_transform(frame)
        _, _, _, _, pose1, _ = get_car_pc_global_pc_flow_transform(dataset_pb2.Frame.FromString(bytearray(all_data[data_idx+1])))
        ...
        with h5py.File(output_dir/f'{scene_id}.h5', 'a') as f:
            ...
            create_group_data(group, car_frame_pc, pose, ego_motion=ego_motion, gm=np.array(ground_mask), flow_0to1=(flow/10.0+pose_flow), flow_category=label, beam_id=beam_id)
        # if data_idx > 10:
        #     break
